...
import type { Widget, WidgetDisplayType } from "@/hooks/useWidgets";
...
const WidgetDialog = ({ open, onOpenChange, onSubmit, editingWidget }: WidgetDialogProps) => {
  ...
  useEffect(() => {
    if (editingWidget) {
      ...
      setColSpan(String(editingWidget.colSpan || 1));
      setRowSpan(String(editingWidget.rowSpan || 1));
      ...
      setTitle(""); setDisplayType("key-value"); setApiUrl(""); setJsonPath("");
      setPollInterval("10"); setColSpan("1"); setRowSpan("1"); setIconName("BarChart3");
      setTitleField(""); setSubtitleField(""); setLabelField(""); setValueField("");
      setGaugeValueField(""); setGaugeMax("100"); setGaugeLabel("%");
    }
  }, [editingWidget, open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-card !bg-card border-border sm:max-w-md max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-foreground">{isEditing ? "Edit Widget" : "Add Widget"}</DialogTitle>
          <DialogDescription>{isEditing ? "Update this widget's configuration." : "Add a new widget to display data from an API."}</DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label className="text-muted-foreground">Title</Label>
            <Input placeholder="e.g. Recently Added" value={title} onChange={(e) => setTitle(e.target.value)} required className="bg-secondary border-border" />
          </div>
          ...
          <div className="space-y-3 pt-2 border-t border-border">
            <p className="font-mono text-[10px] text-muted-foreground/50 uppercase tracking-widest">Data Source</p>
            <div className="space-y-2">
              <Label className="text-muted-foreground">API URL</Label>
              <Input placeholder="https://api.jambiya.me/proxy?target=..." value={apiUrl} onChange={(e) => setApiUrl(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label className="text-muted-foreground">JSON Path</Label>
                <Input placeholder="e.g. data.items" value={jsonPath} onChange={(e) => setJsonPath(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
              </div>
              <div className="space-y-2">
                <Label className="text-muted-foreground">Poll (seconds)</Label>
                <Input type="number" min={1} placeholder="10" value={pollInterval} onChange={(e) => setPollInterval(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
              </div>
            </div>
          </div>
          ...
          {(displayType === "list" || displayType === "key-value") && (
            <div className="space-y-3 pt-2 border-t border-border">
              <p className="font-mono text-[10px] text-muted-foreground/50 uppercase tracking-widest">Field Mappings</p>
              {displayType === "list" && (
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Label className="text-muted-foreground text-xs">Title field</Label>
                    <Input placeholder="e.g. Name" value={titleField} onChange={(e) => setTitleField(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-muted-foreground text-xs">Subtitle field</Label>
                    <Input placeholder="e.g. DateCreated" value={subtitleField} onChange={(e) => setSubtitleField(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                  </div>
                </div>
              )}
              {displayType === "key-value" && (
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="e.g. label" value={labelField} onChange={(e) => setLabelField(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-muted-foreground text-xs">Value field</Label>
                    <Input placeholder="e.g. value" value={valueField} onChange={(e) => setValueField(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                  </div>
                </div>
              )}
            </div>
          ...
          {displayType === "gauge" && (
            <div className="space-y-3 pt-2 border-t border-border">
              <p className="font-mono text-[10px] text-muted-foreground/50 uppercase tracking-widest">Gauge Config</p>
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-xs">Value field</Label>
                  <Input placeholder="percent" value={gaugeValueField} onChange={(e) => setGaugeValueField(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-xs">Max</Label>
                  <Input type="number" placeholder="100" value={gaugeMax} onChange={(e) => setGaugeMax(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                </div>
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-xs">Unit label</Label>
                  <Input placeholder="%" value={gaugeLabel} onChange={(e) => setGaugeLabel(e.target.value)} className="bg-secondary border-border font-mono text-xs" />
                </div>
              </div>
            </div>
          )}
